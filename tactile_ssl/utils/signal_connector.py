...
class SignalConnector:
    ...
    def register_signal_handlers(self) -> None:
        ...
        environment = self.trainer._connector.cluster_environment
    ...
    def _slurm_sigusr_handler_fn(self, signum: _SIGNUM, _: FrameType) -> None:
        ...
        self.trainer.save_latest_checkpoint()
    ...
    def _sigterm_notifier_fn(self, signum: _SIGNUM, _: FrameType) -> None:
        log.info(
            rank_prefixed_message(
                f"Received SIGTERM: {signum}", self.trainer.fabric.local_rank
            )
        )
        ...
        if not self.received_sigterm:
            ...
            launcher = self.trainer.fabric.strategy.launcher
